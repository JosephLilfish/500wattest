...
$nr_zarowki = $product->get_attribute( 'pa_zarowki-katalog' );
$zarowki = explode(",",$nr_zarowki);

if($nr_zarowki){
?>

    <h2 class="prod_slick_title">Kompatybilne żarówki</h2>
    <ul class="shop_product in_prod slick_in_prod_custom">
<?php 

foreach($zarowki as $index => $sku){
    global $product;
    $sku= str_replace(' ', '', $sku);
    $slide = wc_get_product_id_by_sku( $sku );
    if(!$slide) continue;
    
    $product = wc_get_product( $slide );
    $title_product = get_the_title($slide);
    $image_product = $product->get_image($slide);
